...
const Navbar = () => {
  ...
  return (
    <nav className="w-full bg-white/30 backdrop-blur-3xl border-b border-gray-100 shadow-sm z-50 sticky top-0">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-20"> {/* Changed justify-center to justify-between */}
        {/* Logo */}
        ...
        {/* Combined Right-aligned items: Burger Menu (mobile) & Contact (desktop) */}
        <div className="flex items-center flex-shrink-0"> {/* This new div acts as the right container */}
          {/* Burger Menu Icon (visible on smaller screens) */}
          <div className="lg:hidden flex items-center"> {/* Removed flex-shrink-0 from here as it's now on the parent */}
            <button onClick={() => setMobileOpen(!mobileOpen)} className="p-2 rounded focus:outline-none text-gray-700 hover:text-blue-700">
              {mobileOpen ? <X className="w-7 h-7" /> : <Menu className="w-7 h-7" />}
            </button>
          </div>
          ...
          {/* Contact & Actions (hidden on mobile, aligned right on desktop) */}
          <div className="hidden lg:flex items-center space-x-2"> {/* Removed flex-shrink-0 from here as it's now on the parent */}
            <Headphones className="w-7 h-7 text-blue-400" />
            <div className="flex flex-col">
