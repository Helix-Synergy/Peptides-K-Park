...
//   const isInView = require('framer-motion').useInView(ref, { once: true });
//   return (
...
//           custom={i}
//           className={
//             'text-indigo-400 text-base font-semibold tracking-widest uppercase ' + className
//           }
//         >
...
//         </motion.h1>
//         <motion.p
//           className="text-lg sm:text-xl text-gray-500 mb-8 max-w-xl"
//           initial="hidden"
//           animate="visible"
//           variants={textVariants}
//           custom={3}
...
import { ArrowRight } from 'lucide-react';
import { motion } from 'framer-motion';
import gsap from 'gsap'; // Make sure GSAP is correctly installed and imported
  ...
  return (
    <div className="flex justify-start mb-4">
      {splittedText.map((current, i) => (
        <motion.div
          key={i}
          ref={ref} // ref is on the container, not individual letters for useInView
          variants={pullupVariant}
          initial="initial"
          animate={isInView ? 'animate' : ''}
          custom={i}
          className={
            'text-sm sm:text-base font-semibold tracking-widest uppercase ' + className // <<< Made text size responsive
          }
        >
          {current === ' ' ? <span>&nbsp;</span> : current}
        </motion.div>
...
const Hero = () => {
  ...
  return (
    <section className="relative w-full min-h-[80vh] flex flex-col lg:flex-row items-stretch bg-white overflow-hidden">
      {/* Left: Text Content */}
      <div className="flex-1 flex flex-col justify-center px-6 sm:px-12 py-12 lg:py-0 z-10 text-center lg:text-left"> {/* Added text alignment for better mobile presentation */}
        {/* Animated subtitle with per-letter pull-up */}
        <div className="lg:justify-start justify-center flex"> {/* Center on mobile, left on desktop */}
           <LettersPullUp text="Empowering Innovation" />
        </div>
        ...
        <motion.p
          className="text-lg sm:text-xl text-gray-500 mb-8 max-w-xl mx-auto lg:mx-0" // Centered on mobile, left on desktop
          initial="hidden"
          animate="visible"
          variants={textVariants}
          custom={3}
        ...
        <motion.button
          className="flex items-center justify-center bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-xl text-lg shadow transition w-fit mx-auto lg:mx-0 min-w-[200px]" // <<< Responsive button width
          whileHover={{ scale: 1.06, boxShadow: '0 8px 32px rgba(91,107,206,0.18)' }}
          whileTap={{ scale: 0.97 }}
          initial={{ opacity: 0, y: 40 }}
          animate={{ opacity: 1, y: 0 }}
      ...
      {/* Right: Diagonal Image Split */}
      {/* The desktop image retains its properties, hiding on 'lg' breakpoint */}
      <div className="relative flex-1 min-h-[350px] hidden lg:block">
        <div className="absolute inset-0 w-full h-full">
          {/* Diagonal overlay */}
          <svg className="absolute inset-0 w-full h-full" viewBox="0 0 800 800" fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none">
          ...
          <motion.img
            ...
            alt="Team working"
            className="absolute top-0 right-0 w-full h-full object-cover object-center [clip-path:polygon(30%_0,100%_0,100%_100%,0_100%)]"
            style={{ clipPath: 'polygon(30% 0, 100% 0, 100% 100%, 0 100%)' }} // inline style for clip-path for broader compatibility
            initial={{ x: 600, opacity: 0, scale: 1.05 }}
            animate={{ x: 0, opacity: 1, scale: 1 }}
            transition={{ delay: 0.3, duration: 1, ease: 'easeOut' }}
          />
      ...
      <div className="lg:hidden w-full h-64 relative"> {/* h-64 is a fixed height, consider h-[50vh] or a responsive height if needed */}
        <motion.img
          src="https://images.unsplash.com/photo-1519389950473-47ba0277781c?auto=format&fit=crop&w=800&q=80"
          alt="Team working"
          className="w-full h-full object-cover object-center rounded-t-2xl"
